...
export const SolutionsPageThree: React.FC = () => {
  return (
    <div className="min-h-screen bg-white text-[#0f172a] font-sans relative">
      <Navbar />
      ...
      <main className="max-w-[1600px] mx-auto px-6 md:px-12 lg:px-16 pb-24 relative z-10">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-24 mt-8">
          {CAPABILITIES.map((sol) => (
            <div 
              key={sol.title}
        ...
        <section className="border-t border-slate-100 pt-16 mb-16">
          <p className="text-[11px] font-bold tracking-[0.25em] text-slate-500 uppercase block mb-3">
            Case Blueprints
          </p>
          <h2 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-6">
            Applied AI &amp; Data Architectures
          </h2>
          <p className="text-slate-700 text-lg md:text-xl leading-relaxed max-w-4xl font-normal">
            A selection of applied program blueprints demonstrating how core data engineering capabilities and agentic AI architectures are synthesized to solve specific corporate challenges.
          </p>
        </section>

        {/* Detailed Case Use Cards */}
        <div className="space-y-24 max-w-7xl mx-auto pt-8">
          {useCases.map(({ number, category, title, description, detail, stack, Diagram, accent }, idx) => (
            <motion.div
              key={number}
              custom={idx}
              variants={fadeUp}
            ...
            >
              ...
              <div className={idx % 2 === 1 ? 'md:order-2' : 'md:order-1'}>
                ...
                <h2
                  className="font-display font-bold leading-tight text-[#0f172a] mb-4"
              ...
              {/* Diagram side */}
              <div className={`${idx % 2 === 1 ? 'md:order-1' : 'md:order-2'} relative`}>
                <div
